...
class node_controller():
    ...
    def check_sim(self):
        for i in list(self.action_info.keys()):
            print("for: ", i, (i in self.action_info))
            if i in self.action_info:
                ...
                try:
                    ...
                    if res_dict['exec'][0] > 0 or res_dict['exec'][1] > 0 or  res_dict['lender'] > 0 or  res_dict['renter'] > 0:
                        continue
...
test.packages_reload(['video', 'disk', 'couchdb_test', 'image', 'linpack', 'markdown2html', 'matmul'])
test.print_info()

# a Flask instance.
proxy = Flask(__name__)
container_port_number_count = 18211
       ...
       # test.image_base(action_name)
        while True:
            try:
                ...
                res = requests.post(url, json = {"action": action_name, "pwd": action_name, "QOS_time": 1.5, "QOS_requirement": 0.95, "min_port": container_port_number, "max_container": 10})
                if res.text == 'OK':
                    break
            except Exception:
                time.sleep(0.01)       
...
def main():
    server = WSGIServer(('172.23.164.202', node_port), proxy)
